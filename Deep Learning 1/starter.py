...
def FirstLayer(net, l2_reg_val, is_training):
  ...
  """
    ...
    l2_reg = l2_reg_val * tf.reduce_sum(net * net)
...
def EmbeddingL2RegularizationUpdate(embedding_variable, net_input, learn_rate, l2_reg_val):
  ...
  Returns tf op that applies one regularization step on embedding_variable."""
    ...
    intermediate_mat = tf.matmul(
        tf.matmul(tf.transpose(net_input), net_input),
        embedding_variable)
    intermediate_mat = 2*learn_rate*l2_reg_val*intermediate_mat
    return embedding_variable.assign(embedding_variable-intermediate_mat)
...
def EmbeddingL1RegularizationUpdate(embedding_variable, net_input, learn_rate, l1_reg_val):
  ...
  Returns tf op that applies one regularization step on embedding_variable."""
    ...
    intermediate_mat = tf.matmul(tf.transpose(net_input),
                                 tf.math.sign(
                                     tf.matmul(net_input, embedding_variable)
                                 )
                                 )
    intermediate_mat = learn_rate * l1_reg_val * intermediate_mat

    return embedding_variable.assign(embedding_variable - intermediate_mat)


# ** TASK 3
def SparseDropout(slice_x, keep_prob=0.2):
  ...
  """
    nonzero_x_indices = numpy.transpose(numpy.nonzero(slice_x))
    # print(nonzero_x_indices)
    shuffler = numpy.ones(len(nonzero_x_indices))
    shuffler[0: int((1 - keep_prob) * len(nonzero_x_indices))] = 0
    numpy.random.shuffle(shuffler)
    zero_indices_of_shuffler = numpy.where(shuffler == 0)[0]
    rows, cols = numpy.transpose(nonzero_x_indices[numpy.transpose(zero_indices_of_shuffler)])
    slice_x[rows, cols] = 0
    return slice_x
...
def ComputeTSNE(embedding_matrix):
  ...
  """
    embeds_2d = TSNE(learning_rate=50, n_iter=650).fit_transform(embedding_matrix)
    return embeds_2d
...
def VisualizeTSNE(sess):
    ...
    f = plot.figure()
    for category, words in class_to_words.items():
        for w in words:
            x, y = tsne_embeddings[TERM_INDEX[w]]
            plot.scatter(x, y, c=class_to_color[category])
            plot.annotate(w, (x, y))
...
def BuildInferenceNetwork(x, l2_reg_val, is_training):
  ...
  """
    ...
    # EMBEDDING_VAR = tf.get_variable(name="fully_connected/weights:0", reuse=tf.AUTO_REUSE)
